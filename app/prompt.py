...
def find_property_uuid_column(schema):
	E='partitions';D='columns';C='property';B='property_uuid';A='name'
	for(table_name,meta)in schema.items():
		for col in meta.get(D,[]):
			if col[A].lower()==B:return B
		for part in meta.get(E,[]):
			if part[A].lower()==B:return B
	for(table_name,meta)in schema.items():
		for col in meta.get(D,[]):
			name_lower=col[A].lower()
			if C in name_lower and'uuid'in name_lower:return col[A]
	for(table_name,meta)in schema.items():
		for col in meta.get(D,[]):
			if col[A].lower()==C:return col[A]
		for part in meta.get(E,[]):
			if part[A].lower()==C:logger.info("Found 'property' as partition key — using for filtering");return C
def _match_hotel_name(text,property_names):
	if not property_names:return
	text_lower=text.lower()
	for name in property_names:
		if name.lower()in text_lower:return name
	for name in property_names:
		significant=[w for w in name.lower().split()if w not in('the','a','an')]
		if len(significant)>=2 and all(w in text_lower for w in significant):return name
def _build_chart_hint(display_type):
	if display_type=='bar':return'\nCHART STRUCTURE (bar chart): Return exactly 2 columns — a category label column and a numeric value column (COUNT/SUM/AVG). ORDER BY the value column DESC so bars are sorted largest to smallest.\nExample: SELECT category_name, COUNT(*) AS count FROM ... GROUP BY 1 ORDER BY 2 DESC LIMIT 100'
	if display_type=='line':return"\nCHART STRUCTURE (line chart): Return exactly 2 columns — a time bucket column using DATE_TRUNC (x-axis) and a numeric value column (y-axis). ORDER BY the time column ASC so the trend flows left to right.\nExample: SELECT DATE_TRUNC('month', created_date) AS month, COUNT(*) AS count FROM ... GROUP BY 1 ORDER BY 1 LIMIT 100"
	return''
def _format_enum_section(column_values):
	...
	for(col,vals)in column_values.items():
		if vals:lines.append(f"- {col}: {', '.join(repr(v)for v in vals)}")
	return _B.join(lines)
def _build_maintenance_instructions(property_restriction,entity_context,enum_hint,chart_hint=''):return f"""
...
def _build_incident_instructions(property_restriction,entity_context,enum_hint,chart_hint=''):return f"""
Use Amazon Redshift SQL syntax. Output ONLY the SQL query, no explanation.
...
def _build_common_parts(text,context,sql,redshift_target,property_uuid=_A,user_uuid=_A):
	A='incident';schema=load_schema(redshift_target);ddl_schema=schema_to_ddl(schema);column_values=load_column_values(redshift_target);enum_section=_format_enum_section(column_values);property_uuids=[u.strip()for u in property_uuid.split(',')if u.strip()]if property_uuid else[];normalized_text,matched_entities,entity_hints=preprocess_query(text);property_col=find_property_uuid_column(schema)
	if property_col:logger.info(f"Detected property UUID column: '{property_col}'")
	else:logger.warning('Could not detect property UUID column from schema')
	property_restriction=''
	if property_uuids and property_col:uuid_in_list=', '.join(f"'{u}'"for u in property_uuids);property_restriction=f"\nCRITICAL: Every query MUST include WHERE {property_col} IN ({uuid_in_list}). This is a mandatory access control filter — never omit it."
	elif property_uuids and not property_col:logger.warning(f"Property UUIDs provided ({property_uuids}) but no property UUID column found — skipping filter")
	time_hint=get_time_expression_hint(text)
	if time_hint:entity_hints=(entity_hints+_B+time_hint).strip()if entity_hints else time_hint
	hotel_hint=''
	if redshift_target==A and column_values:
		property_names=column_values.get('property_name',[]);matched_hotel=_match_hotel_name(text,property_names)
		if matched_hotel:hotel_hint=f"property_name = '{matched_hotel}'";entity_hints=(entity_hints+_B+hotel_hint).strip()if entity_hints else hotel_hint;matched_entities=True;logger.info(f"Detected hotel in query: '{matched_hotel}'")
	entity_context=f"\nDETECTED ENTITIES (use these exact values):\n{entity_hints}"if matched_entities or time_hint else'';enum_hint=f"\n{enum_section}"if enum_section else'';detected_display=get_display_type_from_question(text);chart_hint=_build_chart_hint(detected_display)
	if chart_hint:logger.info(f"Injecting chart hint for display type: {detected_display}")
	if redshift_target==A:additional_instructions=_build_incident_instructions(property_restriction,entity_context,enum_hint,chart_hint)
	else:additional_instructions=_build_maintenance_instructions(property_restriction,entity_context,enum_hint,chart_hint)
	return normalized_text,ddl_schema,additional_instructions
def build_prompt(text,context,sql,redshift_target,property_uuid=_A,user_uuid=_A):normalized_text,ddl_schema,additional_instructions=_build_common_parts(text,context,sql,redshift_target,property_uuid,user_uuid);return f"""### Task
...
[SQL]"""
def build_correction_prompt(text,context,sql,redshift_target,failed_sql,error_message,property_uuid=_A,user_uuid=_A):normalized_text,ddl_schema,additional_instructions=_build_common_parts(text,context,sql,redshift_target,property_uuid,user_uuid);error_summary=error_message.split(_B)[0][:300];return f"""### Task
